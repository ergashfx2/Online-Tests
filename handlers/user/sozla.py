...
# from handlers.user.checker import check_base, check_array
...
from data.states import PersonalData, Texts, Test
...
@dp.callback_query_handler(AdminFilter(), text="main",
                           state=["remove_channel", "add_channel", PersonalData.ID, PersonalData.answer,Test.kod,Test.javoblar])
async def add_channel(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer("*Xush kelibsiz admin*", parse_mode="markdown", reply_markup=admin_main)
    await state.finish()
...
@dp.callback_query_handler(AdminFilter(), text="add_test")
async def add_channel(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer("*Test kodini yuboring*", parse_mode="Markdown", reply_markup=cancel)
    await Test.kod.set()
...
@dp.message_handler(AdminFilter(), state=Test.kod)
async def add_channel(msg: types.Message, state: FSMContext):
    await state.update_data({"kod": msg.text})
    if msg.text not in javoblar[0]:
        await msg.answer("*Test javoblarini yuboring*", parse_mode="Markdown", reply_markup=cancel)
        await Test.next()
    else:
        await msg.answer("*Bunday kodli test mavjud\n\nTest kodini yuboring*", parse_mode="Markdown",
                         reply_markup=cancel)


@dp.message_handler(AdminFilter(), state=Test.javoblar)
async def add_channel(msg: types.Message, state: FSMContext):
    await state.update_data({"javoblar": msg.text})
    data = await state.get_data()
    kodi = data.get("kod")
    javoblari = data.get("javoblar")
    db.add_live(kodi, javoblari)
    dict = {kodi: javoblari}
    r = javoblar[0].update(dict)
    print(javoblar)
    await msg.answer(f"*Test kodi* {kodi}\nJavoblari {javoblari}", parse_mode="Markdown", reply_markup=cancel)
    await state.finish()
