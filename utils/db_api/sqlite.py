...
class Database:
    def __init__(self, path_to_db="/home/ergashfx2/DTMBOT/data/main.db"):
...
"""
    ...
    def add_test(self, cid,fmid, mid):
        sql = """INSERT INTO testlar(cid,fmid,mid) VALUES(?, ?,?)"""
        self.execute(sql, parameters=(cid, fmid,mid), commit=True)

    def add_live(self, kod,javoblar):
        sql = """INSERT INTO live(test_kodi,test_javoblari) VALUES(?, ?)"""
        self.execute(sql, parameters=(kod, javoblar), commit=True)
    ...
    def select_test_all(self):
        return self.execute('SELECT * FROM testlar;', fetchall=True)

    def select_live_all(self):
        return self.execute('SELECT * FROM live;', fetchall=True)

    def select_user_all_body(self):
        return self.execute('SELECT cid, full_name, phone, is_blocked,bal FROM Users;', fetchall=True)
    ...
    def update_user_bal(self, bal, cid):
        sql = f"""UPDATE Users SET bal=? WHERE cid=?"""
        return self.execute(sql, parameters=(bal, cid), commit=True)
    ...
    def delete_tests(self):
        return self.execute(f'DELETE FROM testlar', commit=True)
